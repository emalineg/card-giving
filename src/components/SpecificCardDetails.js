  ...
  useEffect(() => {
    if (cardCreated) {
      ...
      if (stamp && sound) {
        ...
        stamp.addEventListener('animationstart', playSound);

       
        return () => {
          stamp.removeEventListener('animationstart', playSound);
        };
      } else {
        console.error('Stamp or sound element not found.');
      }
    }
  }, [cardCreated]); 
    ...
    <div className="SpecificCardDetails">
      {cardCreated ? (
        <div className="card-created-info">
          ...
          <audio id="stamp-sound" src="/stamp.mp3" preload="auto"></audio>
